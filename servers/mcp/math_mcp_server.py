...
def run(transport: Literal["sse", "stdio"] = "stdio", port: int | None = None) -> None:
    ...
    def multiply(a: Annotated[int|float|str, "the first number"], b: Annotated[int|float|str, "the second number"]) -> float:
        return float(a) * float(b)
    
    @kernel_function(description="add two numbers.")
    def add(a: Annotated[int|float|str, "the first number"], b: Annotated[int|float|str, "the second number"]) -> float:
        return float(a) + float(b)

    # kernel.add_service(AzureChatCompletion())
    kernel.add_function("math", multiply, "multiply two numbers")
    kernel.add_function("math", add, "add two numbers")
    server = kernel.as_mcp_server(server_name="math_sk")

...
class RestaurantPlugin:
    ...
    def list_restaurants(self) -> Annotated[str, "Returns a list of available restaurants."]:
        return """
            1. AAA: a classic steakhouse with a rustic atmosphere.
            2. BBB: a seafood restaurant with a view of the ocean.
            3. CCC: a casual eatery with a diverse menu.
            """

    @kernel_function(description="Provides specials of a restaurant.")
    def get_specials(
        self, restaurant: Annotated[Literal["AAA", "BBB", "CCC"], "The name of the restaurant."]
...
async def run(transport: Literal["sse", "stdio"] = "stdio", port: int | None = None) -> None:
    ...
    kernel.add_plugin(RestaurantPlugin(), plugin_name="RestaurantPlugin")
